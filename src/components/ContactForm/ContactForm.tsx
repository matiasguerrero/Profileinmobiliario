...
export default function ContactForm({data}:contactData) {
    ...
    const handleSubmit = (event:any) => {
      ...
      emailjs.send("default_service", "template_d8my5sl", contact, "AacbpR9UvVep_61Dc")
          .then((response) => {
              // console.log("SUCCESS!", response.status, response.text);
        setSendMessageSuccess(true)
         // Limpiar los campos del formulario
        setContact({ userName: "", userEmail: "", message: "" });
        setTimeout(() => {
  ...
  return (
    ...
    <div className='flex flex-col'>
        ...
        <input onChange={handleChange} name='userName' required id='userName' type='text' className={inputClassname}  value={contact.userName}/>
    </div>
    <div className='flex flex-col'>
      <label htmlFor='userEmail' className='font-semibold'>E-mail</label>
      <input onChange={handleChange} required id='userEmail' name='userEmail' type='email' className={inputClassname} value={contact.userEmail}/>
    </div>
    <div className='flex flex-col'>
      <label htmlFor='message' className='font-semibold'>{data.contact_form.message_label}</label>
      <textarea onChange={handleChange} required className={`h-32 ${inputClassname}`} id='message' name='message' value={contact.message}></textarea>
    </div>
    <motion.button 
      type='submit' 
      className='rounded-full w-full text-white transition-all duration-500 font-medium py-3 px-7'
      animate={{ backgroundColor: sendMessageSuccess ? "green" : "black"}}
    > 
